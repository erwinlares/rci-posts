...
For our case scenario, let us assume we have a proper project with some .R code that accomplishes a simple task: the script takes as an input a dataset about labs that use a RCI resource and as an output estimates how frequently that lab uses that particular resource across time.  

The dataset minimally contains as variables an integer `year`, a character `lab_id`, and a character `usage`. `year`. `year` is the year where usage data is recorded. `lab_id` is a character identifier for the lab. `usage` is a twelve-character string that indicates if the lab used the resource in a month of the corresponding `year`; `usage` looks like "FTFFTTTFTFFT", one for every month of the year, "T" if the resource was used, `F` otherwise.
...
plot_usage_pattern <- function(df, lab_id, start_year, start_month, end_year, end_month) {
  pattern <- get_extended_pattern(df, lab_id, start_year, start_month, end_year, end_month)
...
plot_usage_pattern(data, "Lake Mendota Research Collective", 2000, 1, 2001, 6)
